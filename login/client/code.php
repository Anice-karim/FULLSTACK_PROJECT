...
if (isset($_POST['edit_btn'])) {
    ...
    if (isset($_SESSION['email'])) {
        ...
        $selectQuery = "SELECT * FROM assure WHERE email = '$email'";
        ...
        if (!empty($_POST['current_password']) && !empty($_POST['new_password'])) {
            ...
            if ($user && md5($currentPassword) === $user['password']) {
                ...
                $updatePassQuery = "UPDATE assure SET password = '$newPasswordHash' WHERE email = '$email'";
        ...
        if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
            ...
            if (strpos($fileType, 'image') === false) {
                ...
                if (move_uploaded_file($_FILES['profile_pic']['tmp_name'], $targetPath)) {
                    $updatePicQuery = "UPDATE assure SET profile_pic = '$filename' WHERE email = '$email'";
